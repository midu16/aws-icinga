...
$text = decode_json($CDict);

foreach my $section (keys %$text) {
    foreach my $property (keys { $text->{$section} }) {
        print "$section, $property: $text->{$section}->{$property}\n";
    }
}
...
$cd = shift;
     ...
     if (! $cd) {
        FNO::add({ Summary => "No json parameter has been passed",
                   Severity => "Major",
                   AlertGroup => "ICINGA",
...
sub print_usage () {
        print "Usage:\n";
        print "  $0 -C <json format string containing arguments> \n";
        print "\nOptions:\n";
        print "  -h, --help\n";
        print "     Print this message\n";
        print "  -C, --Cdict\n";
        print "     Json format string containing arguments\n";
        print "     Example: {\"eno1\": {\"interface\": \"eno1\",\"App\": \"System\",\"Sev\": \"warning\",\"MsgG\": \"OPS-IS-CLOUD\"}}\n";
}
